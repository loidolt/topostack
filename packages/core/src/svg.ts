...
export function buildFabricationPackage(ir: GeometryIRV1, config: ProjectConfigV1): FabricationPackageV1 {
  ...
  if (ir.vectorStatus !== "available" && (config.showRoads || config.showTrails || config.showWater || config.showWaterDepth)) throw new Error("Transportation and water data is unavailable. Disable those map details or regenerate after the service is restored.");
